...
public class SearchResult {
   ...
   public SearchResult(int score, Line line, int mateIn, int startDepth) {
      ...
      this.score = score;
      this.mateIn = mateIn;
      if (this.mateIn != -1) { System.out.println(count + ", mateIn " + mateIn + ", " + line); }
      if (line == null) {
         this.line = Optional.empty();
      } else {
         if (line.getMoves().size() > startDepth + 1) { throw new RuntimeException("line too long (startDepth: " + startDepth + "): " + line); }
   ...
   public Optional<Line> getLine() { return line; }

   public void setLine(Optional<Line> line) { this.line = line; }
