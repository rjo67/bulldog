...
public class AlphaBeta3 implements SearchStrategy {
    ...
    * @return best result
    */
   private SearchResult alphabeta(Position posn, int depth, int min, int max, Line line, MoveTree moveTree, MiniMax evaluationType) {
      if (depth == 0) {
         NBR_POSNS_EVALUATED++;
      ...
      case MAX:
         for (IMove move : moves) {
            ...
            // if (USE_ZOBRIST && !foundZobrist) { zobristMap.updateZobristMap(newPosn, depth, result); }
            if (result.getScore() > min) {
               ...
               if (result.getLine().isPresent()) {
                  ...
                  if (depth == startDepth) { result.printUCI(outputStream); }
               }
   ...
   @Override
   public int getCurrentNbrNodesSearched() { return NBR_NODES_SEARCHED; }

   @Override
   public int getCurrentDepth() { return startDepth; }
   ...
   enum EvalType {
      ...
      public static String print(Set<EvalType> evaluations) {
         ...
         for (EvalType eval : evaluations) {
            if (eval != NORMAL) { sb.append(eval).append(" "); }
         }
         return sb.toString();
      }
   }
   ...
   static class MoveTree {
      ...
      public String toString() {
         ...
         if (type == null) {
            ...
            sb.append(String.format("%s %s %d %s (min:%d max:%d) %d %s%n", type, "        ".substring(0, 9 - depth), depth, move, min, max, score,
                  EvalType.print(evaluations)));
         }
