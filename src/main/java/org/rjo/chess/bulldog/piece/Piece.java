...
public enum Piece {
	...
	PAWN(100, false, "", false, false, new int[] {}, // offsets not used for pawns
			/*
			 * Firstly the shelter in front of white short castle (long castle - it's
			 * symmetrical) - pawns at f2, g2 and h2 get bonuses. Additionally we set
			 * negative values for f3 and smaller for g3 which both create holes around
			 * king. Pawn h2 have the same value on h2 and h3, so the engine may create the
			 * hole if the situation calls for it. Moreover - if it gets position with a
			 * pawn at g3 and a bishop at g2, then it still may play h3 or not. Therefore h3
			 * has the same value as h2.
			 * 
			 * Zero value on f4, g4, h4 prevents playing with pawns in front of the king.
			 * Moving these pawns to f5, g5, h5 still brings nothing, but at this moment we
			 * have the same values as on rank 2.
			 * 
			 * In the centre we have the most negative values on d2 and e2. We don't like
			 * these pawns. d3 and e3 aren't good either. Only d4 and e4 in the centre. Even
			 * better on d5 and e5.
			 * 
			 * Beginning with rank 6th we give bonus for advanced pawns. On rank 7th even
			 * bigger.
			 */
			// @formatter:off
         new int[] {
	...
	ROOK(500, true, "R", true, false, new int[] { -10, -1, 1, 10 },
			/*
			 * Ideas: to centralize, occupy the 7th rank and avoid a, h columns (in order
			 * not to defend pawn b3 from a3).
			 */
			// @formatter:off
         new int[] {
        		  0,  0,  0,  0,  0,  0,  0,  0,
	...
	KNIGHT(320, false, "N", false, false, new int[] { -21, -19, -12, -8, 8, 12, 19, 21 },
			 ...
			 * edge is a bad idea. Standing in the corner is a terrible idea.
			 */
			// @formatter:off
         new int[] {
	...
	BISHOP(330, true, "B", false, true, new int[] { -11, -9, 9, 11 },
			/*
			 * We avoid corners and borders. Additionally we prefer squares like b3, c4, b5,
			 * d3 and the central ones.
			 */
			// @formatter:off
         new int[] {
	...
	QUEEN(900, true, "Q", true, true, new int[] { -11, -10, -9, -1, 1, 9, 10, 11 },
			/*
			 * Prefer to keep the queen in the centre and b3, c2 squares
			 */
			// @formatter:off
         new int[] {
	...
	KING(20000, false, "K", false, false, new int[] { -11, -10, -9, -1, 1, 9, 10, 11 },
			 ...
			 * 
			 * There are different values for the endgame, these are stored from offset 64.
			 * (0..63 normal, 64..127 endgame).
			 * 
			 * TODO need to reference these in the position evaluator
			 */
			// @formatter:off
         new int[]{
	 ...
	 * https://www.chessprogramming.org/Simplified_Evaluation_Function
	 * 
	 * Important: array value [0] corresponds to square a8; [63] == h1. As defined
	 * in Board.Square. For black these values must be mirrored.
	 * 
	 */
	...
	private Piece(int pieceValue, boolean slidingPiece, String symbol, boolean movesHorizontallyOrVertically,
			boolean movesDiagonally, int[] moveOffsets, int[] squareValues) {
	...
	/**
	 * Returns the FEN symbol for this piece. This is usually the 'symbol' in upper
	 * or lower case. Exception is the pawn.
	 */
	public String fenSymbol(Colour colour) {
	...
	public boolean canSlideAlongRay(Ray ray) {
		return ((ray.isHorizontal() || ray.isVertical()) && slidesHorizontallyOrVertically)
				|| (ray.isDiagonal() && slidesDiagonally());
	}
	...
	public static Piece convertStringToPieceType(char ch) {
		...
		for (Piece p : Piece.values()) {
			...
			if (ch == p.symbol.charAt(0) || ch == p.symbol.toLowerCase().charAt(0)) {
				return p;
			}
		}
