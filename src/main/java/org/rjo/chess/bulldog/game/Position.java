...
public class Position {
    ...
    * Copy constructor.
    * 
    * All information in data structures is "shallow" cloned. If it gets changed later the appropriate data structures must be fully cloned.
    * 
   ...
   /**
    * Calculates horizontal/vertical and diagonal pin masks, emenating from the given king's square.
    * 
   ...
   public void createPinInfo(BitSetUnifier pinMaskHV, BitSetUnifier pinMaskDiag, int kingsSquare, Colour myColour) {
      for (Ray ray : Ray.RAY_TYPES_HORIZONTAL_VERTICAL) {
         ...
         // record the pin
         if (foundMyPiece && foundEnemyPiece) { pinMaskHV.or(tmpBitset); }
      }
      for (Ray ray : Ray.RAY_TYPES_DIAGONAL) {
         ...
         // record the pin
         if (foundMyPiece && foundEnemyPiece) { pinMaskDiag.or(tmpBitset); }
      }
   }
   ...
   public PieceSquareInfo opponentsPieceOnRay(Colour myColour, int startSq, Ray ray, int squareToIgnore) {
      ...
      for (int potentialEnemySq : Ray.raysList[startSq][ray.ordinal()]) {
         ...
         if (myColour.opposes(Pieces.colourOf(pieceAtSquare))) { enemyPiece = Pieces.toPiece(pieceAtSquare); }
    ...
    * <p>
    * <B>algorithm has been changed, this does not currently apply:</B>In order for NegaMax to work, it is important to return the score
   ...
   public int evaluate() {
      ...
      for (int sq = 0; sq < 64; sq++) {
         ...
         if (piece != 0) {
            ...
            materialScore += pieceType.calculatePieceSquareValue(sq, pieceColour);
         }
      }
      ...
      if (getSideToMove() != Colour.WHITE) { enpassantSquare = prevEnpassantSquare; }
      ...
      if (getSideToMove() != Colour.BLACK) { enpassantSquare = prevEnpassantSquare; }
