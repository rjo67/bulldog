...
public class Move implements IMove {
   ...
   public String toUCIString() {
      if (isKingssideCastling()) {
         return (Square.toSquare(originSq) == Square.e1 ? "e1g1" : "e8g8");
      } else if (isQueenssideCastling()) {
         return (Square.toSquare(originSq) == Square.e1 ? "e1c1" : "e8c8");
      } else {
         return Square.toSquare(originSq).name() + Square.toSquare(targetSq).name();
      }
   }
